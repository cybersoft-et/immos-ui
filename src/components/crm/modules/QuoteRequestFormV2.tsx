...
import { MessageSquare, X, Plus, Upload, Trash2 } from 'lucide-react';
import { AgGridReact } from 'ag-grid-react';
import 'ag-grid-community/styles/ag-grid.css';
import 'ag-grid-community/styles/ag-theme-alpine.css';
import { useNavigate } from 'react-router-dom';

export default function QuoteRequestFormV2 () {
  ...
  const handleFileUpload = async (e) => {
    ...
    for (const file of files) {
      ...
      // For now, we'll simulate it with a mock upload
      const formData = new FormData();
      formData.append('Content', file);
      formData.append('Name', formData.Name);
      formData.append('Description', formData.Description);
      formData.append('DocumentTypeIdOrName', 1);
      
      try {
        // Simulate file upload - replace with your actual API endpoint
        const response = await fetch(`${API_BASE}/Quotations/${QuoteNumber}/attachment`, {
            method: 'PUT',
            headers: {
        ...
        // Mock response with documentId
        const mockDocumentId = Math.floor(Math.random() * 100) + 1;
        ...
        // Add to attachments array for submission
        setAttachments(prev => [...prev, { documentId: mockDocumentId }]);
        console.log('Uploaded file:', file.name, 'with documentId:', mockDocumentId);
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert('Quote request ready to submit! Check console for payload.');
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-orange-600 mb-6">Request A Quote Trial</h1>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          ...
          {uploadedFiles.length > 0 && (
            <div className="mt-6">
              <h3 className="text-sm font-medium text-gray-700 mb-3">Uploaded Files ({uploadedFiles.length})</h3>
